...
def makeNameEMJ(self, num):
  return self.name + "_part-" + str(num)


protoJob.makeName = makeNameEMJ


class jobSubmitterEMJ(jobSubmitter):
  def __init__(self):
    super(jobSubmitterEMJ, self).__init__()

    self.helper = emjHelper()

  def addDefaultOptions(self, parser):
    super(jobSubmitterEMJ, self).addDefaultOptions(parser)
    parser.add_option("-y",
                      "--getpy",
                      dest="getpy",
                      default=False,
                      action="store_true",
                      help=('make python file list for ntuple production'
                            ' (default = %default)'))
    parser.add_option("--actualEvents",
                      dest="actualEvents",
                      default=False,
                      action="store_true",
                      help=('count actual number of events from each '
                            'input file (for python file list) '
                            '(default = %default)'))
    self.modes.update({"getpy": 1, })

  def addExtraOptions(self, parser):
    super(jobSubmitterEMJ, self).addExtraOptions(parser)

    parser.add_option("-d",
                      "--dicts",
                      dest="dicts",
                      default="",
                      help=('file with list of input dicts; each dict contains '
                            'signal parameters (required) '
                            '(default = %default)'))
    parser.add_option("-o",
                      "--output",
                      dest="output",
                      default="",
                      help=('path to output directory in which root '
                            'files will be stored (required) '
                            '(default = %default)'))
    parser.add_option("-E",
                      "--maxEvents",
                      dest="maxEvents",
                      default=1,
                      help=('number of events to process per job '
                            '(default = %default)'))
    parser.add_option("-F",
                      "--firstPart",
                      dest="firstPart",
                      default=1,
                      help=('first part to process, in case extending '
                            'a sample (default = %default)'))
    parser.add_option("-N",
                      "--nParts",
                      dest="nParts",
                      default=1,
                      help='number of parts to process (default = %default)')
    parser.add_option("-K",
                      "--skipParts",
                      dest="skipParts",
                      default="",
                      help=('comma-separated list of parts to skip'
                            ', or auto (default = %default)'))
    parser.add_option("--indir",
                      dest="indir",
                      default="",
                      help='input file directory (LFN) (default = %default)')
    parser.add_option("--redir",
                      dest="redir",
                      default="root://cmseos.fnal.gov/",
                      help='input file redirector (default = %default)')
    parser.add_option("--inpre",
                      dest="inpre",
                      default="",
                      help='input file prefix (default = %default)')
    parser.add_option("--outpre",
                      dest="outpre",
                      default="",
                      help='output file prefix (required) (default = %default)')
    parser.add_option("--config",
                      dest="config",
                      default="",
                      help='CMSSW config to run (required) (default = %default)')
    parser.add_option("-A",
                      "--args",
                      dest="args",
                      default="",
                      help=('additional common args to use for all jobs '
                            '(default = %default)'))
    parser.add_option("-v",
                      "--verbose",
                      dest="verbose",
  ...
  def runPerJob(self, job):
    super(jobSubmitterEMJ, self).runPerJob(job)
    if self.getpy:
      self.doPy(job)

  def checkDefaultOptions(self, options, parser):
    super(jobSubmitterEMJ, self).checkDefaultOptions(options, parser)
  ...
  def checkExtraOptions(self, options, parser):
    super(jobSubmitterEMJ, self).checkExtraOptions(options, parser)
    ...
    if options.skipParts == "auto" and (
        len(options.inpre) == 0 or len(options.indir) == 0 or
        (options.indir.startswith("/store/") and len(options.redir) == 0)):
  ...
  def generateExtra(self, job):
    super(jobSubmitterEMJ, self).generateExtra(job)
    job.patterns.update([
  ...
  def generateSubmission(self):
    # get dicts
    flist = open(self.dicts, 'r').read()
    flist = eval(flist)
    # loop over dicts
    for (mX, mDPi, tauDPi) in [(mXs, mDPis, tauDPis) for mXs in flist['mX']
                               for mDPis in flist['mDPi']
                               for tauDPis in flist['tauDPi']]:
      # create protojob
      job = protoJob()
      # extra attribute to store actual events
      if self.actualEvents: job.actualEvents = 0
      # make name from params
      self.helper.setModel(mX=mX, mDPi=mDPi, tauDPi=tauDPi)
      ...
      if self.skipParts == "auto":
        injob = protoJob()
        injob.name = self.helper.getOutName(int(self.maxEvents),
                                            outpre=self.inpre)
        infiles = {
            x.split('/')[-1].replace(".root", "")
            for x in (filter(
                None,
                os.popen("xrdfs " + self.redir + " ls " +
                         self.indir).read().split('\n')) if self.indir.
                      startswith("/store/") else glob(self.indir + "/*.root"))
        }
      ...
      if self.prepare:
        with open("input/args_" + job.name + ".txt", 'w') as argfile:
          arglist = [
              "mX=" + str(mX), "mDPi=" + str(mDPi),
              "tauDPi=" + str(tauDPi),
              "maxEvents=" + str(self.maxEvents), "outpre=" + self.outpre,
              "config=" + self.config,
          ]
          if len(self.indir) > 0:
      ...
      for iJob in xrange(int(self.nParts)):
        ...
        if (self.skipParts == "auto"
            and injob.makeName(iActualJob) not in infiles) or (type(
                self.skipParts) == set and iActualJob in self.skipParts):
          if self.verbose: print "  skipping part " + str(iActualJob)
          continue

        if self.actualEvents:
          from ROOT import TFile, TTree
          iFile = TFile.Open(self.redir + self.indir + "/" +
                             injob.makeName(iActualJob) + ".root")
  ...
  def doPy(self, job):
    ...
    with open(self.getpy_weights, 'a') as wfile:
      nEvents = job.actualEvents if self.actualEvents else int(
          self.maxEvents) * len(job.nums)
      line = '        MCSample("' + job.name + '", "", "", "Constant", ' + str(
          nEvents) + '),'
      wfile.write(line + "\n")
